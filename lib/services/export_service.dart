...
Future<String> _generateCsvParcelaDataInIsolate(
    _CsvParcelaPayload payload) async {
...
Future<String> _generateCsvCubagemDataInIsolate(
    _CsvCubagemPayload payload) async {
  proj4.Projection.add('EPSG:4326', '+proj=longlat +datum=WGS84 +no_defs');
  payload.proj4Defs.forEach((epsg, def) {
    proj4.Projection.add('EPSG:$epsg', def);
  });
  ...
  rows.add([
    ...
    'Northing_Cubagem',
    'Observacao_Cubagem',
    'altura_medicao_secao_m',
    'circunferencia_secao_cm',
    'casca1_mm',
    'casca2_mm',
    'dsc_cm'
  ]);
  ...
  for (var cMap in payload.cubagensMap) {
    ...
    if (secoes.isEmpty) {
      rows.add([
        ...
        arvore.classe,
        formatValue(arvore.alturaTotal, nf2),
        arvore.tipoMedidaCAP,
        formatValue(arvore.valorCAP, nf2),
        formatValue(arvore.alturaBase, nf2),
        dataColetaFormatada,
        easting,
        northing,
        arvore.observacao,
      ...
      for (var secao in secoes) {
        rows.add([
          ...
          northing,
          arvore.observacao,
          formatValue(secao.alturaMedicao, nf2),
          formatValue(secao.circunferencia, nf2),
          formatValue(secao.casca1_mm, nf2),
          formatValue(secao.casca2_mm, nf2),
          nf2.format(secao.diametroSemCasca)
        ]);
      }
    }
  }
...
Future<String> _generateCsvOperacoesInIsolate(
    _CsvOperacoesPayload payload) async {
  ...
  for (final diarioMap in payload.diariosMap) {
    ...
    final custoTotal = (d.abastecimentoValor ?? 0) +
        (d.pedagioValor ?? 0) +
        (d.alimentacaoRefeicaoValor ?? 0) +
        (d.outrasDespesasValor ?? 0);
...
Future<String> _generateCsvConsolidadoInIsolate(
    _CsvConsolidadoPayload payload) async {
  List<List<dynamic>> rows = [];
  final nf = NumberFormat("#,##0.00", "pt_BR");
  final nfKm = NumberFormat("#,##0.0", "pt_BR");
  ...
  final projetos =
      payload.projetosMap.map((k, v) => MapEntry(k, Projeto.fromMap(v)));
  final atividades =
      payload.atividadesMap.map((k, v) => MapEntry(k, Atividade.fromMap(v)));
  final talhoes =
      payload.talhoesMap.map((k, v) => MapEntry(k, Talhao.fromMap(v)));
  ...
  final totalGastos = (diario.abastecimentoValor ?? 0) +
      (diario.pedagioValor ?? 0) +
      (diario.alimentacaoRefeicaoValor ?? 0);
...
class ExportService {
  ...
  Future<void> exportarRelatorioDiarioConsolidadoCsv({
    ...
    try {
      ...
      final Set<int> talhaoIds = {
        ...parcelas.map((p) => p.talhaoId),
        ...cubagens.map((c) => c.talhaoId)
      }.whereType<int>().toSet();

      final talhoes = (await Future.wait(
              talhaoIds.map((id) => _talhaoRepository.getTalhaoById(id))))
          .whereType<Talhao>()
          .toList();
      final talhoesMap = {for (var t in talhoes) t.id!: t.toMap()};

      final Set<int> atividadeIds =
          talhoes.map((t) => t.fazendaAtividadeId).toSet();
      final atividades = (await _atividadeRepository.getTodasAsAtividades())
      ...
      final projetos =
          (await _projetoRepository.getTodosOsProjetosParaGerente())
              .where((p) => projetoIds.contains(p.id))
              .toList();
      ...
      for (final talhaoId in todosTalhoesTrabalhados) {
        if (!totaisAmostrasPorTalhao.containsKey(talhaoId)) {
          final todasAsParcelasDoTalhao =
              await _parcelaRepository.getParcelasDoTalhao(talhaoId);
          totaisAmostrasPorTalhao[talhaoId] = todasAsParcelasDoTalhao.length;
        }
        if (!totaisCubagensPorTalhao.containsKey(talhaoId)) {
          final todasAsCubagensDoTalhao =
              await _cubagemRepository.getTodasCubagensDoTalhao(talhaoId);
      ...
      final String csvData =
          await compute(_generateCsvConsolidadoInIsolate, payload);
      ...
      await _salvarECompartilharCsv(
          context, csvData, fName, 'Relatório Consolidado - GeoForest');
    } catch (e, s) {
  ...
  Future<void> exportarDiarioDeCampoCsv(
      {required BuildContext context, required DiarioDeCampo diario}) async {
    try {
      if (!await _requestPermission(context)) return;

      final nf = NumberFormat("0.00", "pt_BR");
      final projeto = await _projetoRepository.getProjetoById(diario.projetoId);
      final talhao = diario.talhaoId != null
          ? await _talhaoRepository.getTalhaoById(diario.talhaoId!)
          : null;
      ...
      List<List<dynamic>> rows = [
        ...
        [
          'KM Inicial',
          diario.kmInicial != null ? nf.format(diario.kmInicial) : ''
        ],
        ['KM Final', diario.kmFinal != null ? nf.format(diario.kmFinal) : ''],
        ['Destino', diario.localizacaoDestino],
        [
          'Pedágio (R\$)',
          diario.pedagioValor != null ? nf.format(diario.pedagioValor) : ''
        ],
        [
          'Abastecimento (R\$)',
          diario.abastecimentoValor != null
              ? nf.format(diario.abastecimentoValor)
              : ''
        ],
        ...
        [
          ...
          diario.alimentacaoRefeicaoValor != null
              ? nf.format(diario.alimentacaoRefeicaoValor)
              : ''
        ],
        ...
        [
          ...
          diario.outrasDespesasValor != null
              ? nf.format(diario.outrasDespesasValor)
              : ''
        ],
      ...
      final nomeLiderFormatado =
          diario.nomeLider.replaceAll(RegExp(r'\s+'), '_');
      final dataFormatada = diario.dataRelatorio;
      final fName =
          'diario_de_campo_${nomeLiderFormatado}_${dataFormatada}.csv';

      await _salvarECompartilharCsv(
          context, csvData, fName, 'Diário de Campo - GeoForest');
    } catch (e, s) {
      _handleExportError(context, 'exportar diário de campo', e, s);
    }
  }

  Future<void> exportarDesenvolvimentoEquipes(BuildContext context,
      {Set<int>? projetoIdsFiltrados}) async {
    try {
      if (!await _requestPermission(context)) return;
      ...
      final todosProjetos =
          await _projetoRepository.getTodosOsProjetosParaGerente();
      final projetos =
          (projetoIdsFiltrados == null || projetoIdsFiltrados.isEmpty)
              ? todosProjetos
              : todosProjetos
                  .where((p) => projetoIdsFiltrados.contains(p.id!))
                  .toList();
      ...
      if (projetos.isEmpty) {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content:
                  Text('Nenhum projeto encontrado para o filtro selecionado.'),
              backgroundColor: Colors.orange));
        }
        return;
      }
      ...
      final atividades = todasAtividades
          .where((a) => idsProjetosFiltrados.contains(a.projetoId))
          .toList();
      final atividadesMap = {for (var a in atividades) a.id: a};

      final todosTalhoes = await _talhaoRepository.getTodosOsTalhoes();
      final talhoes = todosTalhoes
          .where((t) => atividadesMap.containsKey(t.fazendaAtividadeId))
          .toList();
      ...
      final parcelasFiltradas =
          todasAsParcelas.where((p) => talhoesMap.containsKey(p.talhaoId));
      ...
      for (final parcela in parcelasFiltradas) {
        ...
        final arvores =
            await _parcelaRepository.getArvoresDaParcela(parcela.dbId!);

        final covas = <String>{};
        for (var a in arvores) {
          covas.add('${a.linha}-${a.posicaoNaLinha}');
        }

        final fustes = arvores
            .where((a) => a.codigo != Codigo.Falha && a.codigo != Codigo.Caida)
            .length;
        final falhas = arvores.where((a) => a.codigo == Codigo.Falha).length;
        final codigosEspeciais = arvores
            .where((a) =>
                a.codigo != Codigo.Normal &&
                a.codigo != Codigo.Falha &&
                a.codigo != Codigo.Caida)
            .length;
      ...
      final cubagensFiltradas =
          todasAsCubagens.where((c) => talhoesMap.containsKey(c.talhaoId));
      ...
      for (final cubagem in cubagensFiltradas) {
        ...
        allColetasData.add({
          ...
          'talhao_area_ha': talhao.areaHa,
          'situacao': cubagem.exportada
              ? 'Exportada'
              : (cubagem.alturaTotal > 0 ? 'Concluida' : 'Pendente'),
          'data_alteracao': null,
      ...
      final String csvData =
          await compute(_generateDevEquipeCsvInIsolate, payload);

      final fName =
          'relatorio_desenvolvimento_equipes_${DateFormat('yyyy-MM-dd_HHmm').format(DateTime.now())}.csv';

      await _salvarECompartilharCsv(context, csvData, fName,
          'Relatório de Desenvolvimento de Equipes - GeoForest');
  ...
  Future<void> _exportarDadosDaEquipe(BuildContext context) async {
    ...
    if (nomeDoColetor == null || nomeDoColetor.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Nome do líder não encontrado.')));
      return;
    }
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Buscando coletas de "$nomeDoColetor"...')));
    final List<Parcela> parcelas = await _parcelaRepository
        .getUnexportedConcludedParcelasByLider(nomeDoColetor);
    ...
    final String fName =
        'geoforest_export_${nomeArquivoColetor}_${DateFormat('yyyy-MM-dd').format(DateTime.now())}.csv';
    ...
    if (context.mounted) {
      ...
      await Share.shareXFiles([XFile(path)],
          subject: 'Exportação GeoForest - Coleta de $nomeDoColetor');

      final idsParaMarcar =
          parcelas.map((p) => p.dbId).whereType<int>().toList();
  ...
  Future<void> _exportarBackupDaEquipe(BuildContext context) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Buscando dados para o backup...')));

    final List<Parcela> parcelas = await _parcelaRepository
        .getConcludedParcelasByLiderParaBackup(nomeDoColetor);

    if (parcelas.isEmpty && context.mounted) {
      ScaffoldMessenger.of(context).removeCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Nenhuma parcela encontrada para seu backup.'),
          backgroundColor: Colors.orange));
      return;
    }

    final String fName =
        'geoforest_MEU_BACKUP_${DateFormat('yyyy-MM-dd_HHmm').format(DateTime.now())}.csv';
    final path = await _gerarCsvParcela(parcelas, fName);

    if (context.mounted) {
      ScaffoldMessenger.of(context).removeCurrentSnackBar();
      await Share.shareXFiles([XFile(path)],
          subject: 'Meu Backup GeoForest - $nomeDoColetor');
    }
  }

  Future<void> _showManagerExportDialog(BuildContext context,
  ...
  Future<void> _executarExportacaoGerente(
      BuildContext context, ExportFilters filters) async {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
    ...
    final String fName =
        'geoforest_${tipo}_${DateFormat('yyyy-MM-dd_HHmm').format(DateTime.now())}.csv';
    final path = await _gerarCsvParcela(parcelasParaExportar, fName);

    if (context.mounted) {
      ScaffoldMessenger.of(context).removeCurrentSnackBar();
      await Share.shareXFiles([XFile(path)],
          subject: 'Exportação do Gerente - GeoForest');
      if (!filters.isBackup) {
        final idsParaMarcar =
            parcelasParaExportar.map((p) => p.dbId).whereType<int>().toList();
        await _parcelaRepository.marcarParcelasComoExportadas(idsParaMarcar);
      }
    }
  }

  Future<String> _gerarCsvParcela(
      List<Parcela> parcelas, String nomeArquivo) async {
    final Map<int, List<Map<String, dynamic>>> arvoresPorParcelaMap = {};
    final Set<int> talhaoIds =
        parcelas.map((p) => p.talhaoId).whereType<int>().toSet();
    ...
    for (final parcela in parcelas) {
      if (parcela.dbId != null) {
        final arvores =
            await _parcelaRepository.getArvoresDaParcela(parcela.dbId!);
        arvoresPorParcelaMap[parcela.dbId!] =
            arvores.map((a) => a.toMap()).toList();
      }
    }

    // --- NOVA LÓGICA PARA PEGAR O NOME DA EMPRESA ---
    String empresaFinal = "N/A";
    if (parcelas.isNotEmpty) {
      // Busca o projeto através da primeira parcela da lista
      final projeto =
    ...
    final payload = _CsvParcelaPayload(
      ...
      nomeZona: prefs.getString('zona_utm_selecionada') ??
          'SIRGAS 2000 / UTM Zona 22S',
      proj4Defs: proj4Definitions,
      nomeEmpresa: empresaFinal, // <--- PASSA O VALOR AQUI
    );
  ...
  Future<void> exportarNovasCubagens(BuildContext context) async {
    try {
      ...
      if (cargo == 'gerente') {
        ...
        final cubagensConcluidas =
            cubagens.where((c) => c.alturaTotal > 0).toList();
  ...
  Future<void> _showManagerCubagemExportDialog(BuildContext context,
      {required bool isBackup}) async {
    final todosProjetos =
        await _projetoRepository.getTodosOsProjetosParaGerente();
  ...
  Future<void> _executarExportacaoGerenteCubagem(
      BuildContext context, ExportFilters filters) async {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
    ...
    if (cubagensParaExportar.isEmpty && context.mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text(
              'Nenhuma cubagem encontrada para os filtros selecionados.')));
      return;
    }

    final tipo =
        filters.isBackup ? "BACKUP_CUBAGEM_GERENTE" : "EXPORT_CUBAGEM_GERENTE";
    final String fName =
        'geoforest_${tipo}_${DateFormat('yyyy-MM-dd_HHmm').format(DateTime.now())}.csv';

    await _gerarCsvCubagem(
        context, cubagensParaExportar, fName, !filters.isBackup);
  }

  Future<void> _gerarCsvCubagem(
      BuildContext context,
      List<CubagemArvore> cubagens,
      String nomeArquivo,
      bool marcarComoExportado) async {
    if (cubagens.isEmpty) {
      if (context.mounted)
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
    ...
    if (cubagens.isNotEmpty) {
      // Usamos o repositório para achar o projeto da primeira árvore da lista
      final projeto =
          await _projetoRepository.getProjetoPelaCubagem(cubagens.first);
      empresaFinal = projeto?.empresa ?? "N/A";
    }
    // ------------------------------------------------
    ...
    if (context.mounted)
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Gerando CSV de cubagens...')));

    final Set<int> talhaoIds =
        cubagens.map((c) => c.talhaoId).whereType<int>().toSet();
    final Map<int, Map<String, dynamic>> talhoesMapParaIsolate = {};
    for (final talhaoId in talhaoIds) {
      final talhao = await _talhaoRepository.getTalhaoById(talhaoId);
      if (talhao != null) {
    ...
    for (final cubagem in cubagens) {
      if (cubagem.id != null) {
        final secoes =
            await _cubagemRepository.getSecoesPorArvoreId(cubagem.id!);
        secoesPorCubagemMap[cubagem.id!] =
            secoes.map((s) => s.toMap()).toList();
      }
    }
    ...
    final String csvData =
        await compute(_generateCsvCubagemDataInIsolate, payload);

    // ... resto do código de salvar e compartilhar (permanece igual) ...
    ...
    if (context.mounted) {
      ...
      if (marcarComoExportado) {
        final idsParaMarcar =
            cubagens.map((c) => c.id).whereType<int>().toList();
  ...
  Future<void> exportarAnaliseTalhaoCsv({
    ...
    try {
      ...
      rows.add([
        ...
        nf4.format(analise.areaTotalAmostradaHa)
      ]);
      rows.add(['']);
      ...
      rows.add(
          ['Área Basal (G) m²/ha', nf2.format(analise.areaBasalPorHectare)]);
      ...
      analise.distribuicaoDiametrica.forEach((pontoMedio, contagem) {
        ...
        rows.add([
          '${nf1.format(inicioClasse)} - ${nf1.format(fimClasse)}',
          contagem,
          '${nf1.format(porcentagem)}%'
        ]);
      });

      final hoje = DateTime.now();
      final fName =
          'analise_talhao_${talhao.nome.replaceAll(' ', '_')}_${DateFormat('yyyy-MM-dd_HH-mm').format(hoje)}.csv';
      final csvData =
          const ListToCsvConverter(fieldDelimiter: ';').convert(rows);

      await _salvarECompartilharCsv(
          context, csvData, fName, 'Análise do Talhão ${talhao.nome}');
    } catch (e, s) {
  ...
  Future<void> exportarPlanoDeAmostragem({
    ...
    try {
      ...
      final allParcelas = (await Future.wait(
              parcelaIds.map((id) => _parcelaRepository.getParcelaById(id))))
          .whereType<Parcela>()
          .toList();
      final talhaoIds =
          allParcelas.map((p) => p.talhaoId).whereType<int>().toSet();
      ...
      if (allParcelas.isNotEmpty) {
        final projeto =
            await _projetoRepository.getProjetoPelaParcela(allParcelas.first);
        if (projeto != null) {
      ...
      final projWGS84 = proj4.Projection.get('EPSG:4326')!;
      final projUTM = proj4.Projection.get('EPSG:$codigoEpsg') ??
          proj4.Projection.parse(proj4Definitions[codigoEpsg]!);
      ...
      for (final parcela in allParcelas) {
        ...
        if (parcela.latitude != null && parcela.longitude != null) {
          var pUtm = projWGS84.transform(projUTM,
              proj4.Point(x: parcela.longitude!, y: parcela.latitude!));
          ...
          features.add({
            ...
            'properties': {
              ...
              'data_realizacao': parcela.status == StatusParcela.concluida
      ...
      final hoje = DateTime.now();
      final fName =
          'Plano_Amostragem_${nomeProjetoFinal.replaceAll(' ', '_')}_${DateFormat('yyyyMMdd_HHmm').format(hoje)}.json';
  ...
  Future<void> exportarTudoComoZip({
    ...
    try {
      ...
      await _gerarCsvParcelasParaZip(
          context, talhoes, '${pastaDeExportacao.path}/parcelas_coletadas.csv');
      await _gerarCsvCubagensParaZip(context, talhoes,
          '${pastaDeExportacao.path}/cubagens_realizadas.csv');
  ...
  Future<void> _gerarCsvParcelasParaZip(
      BuildContext context, List<Talhao> talhoes, String filePath) async {
    final List<Parcela> todasAsParcelas = [];
    for (final talhao in talhoes) {
      final talhaoId = talhao.id;
      if (talhaoId != null) {
        final parcelasDoTalhao =
            await _parcelaRepository.getParcelasDoTalhao(talhaoId);
        todasAsParcelas.addAll(
            parcelasDoTalhao.where((p) => p.status == StatusParcela.concluida));
      }
    }
  ...
  Future<void> _gerarCsvCubagensParaZip(
      BuildContext context, List<Talhao> talhoes, String outputPath) async {
    final List<CubagemArvore> todasAsCubagens = [];
    for (final talhao in talhoes) {
      final talhaoId = talhao.id;
      if (talhaoId != null) {
        final cubagensDoTalhao =
            await _cubagemRepository.getTodasCubagensDoTalhao(talhaoId);
        todasAsCubagens
            .addAll(cubagensDoTalhao.where((c) => c.alturaTotal > 0));
      }
    }
  ...
  void _handleExportError(
      BuildContext context, String action, Object e, StackTrace s) {
  ...
  Future<String> _salvarEObterCaminho(
    ...
    await File(path)
        .writeAsBytes(fileName.endsWith('.csv') ? [...bom, ...bytes] : bytes);
    return path;
  }
  ...
  Future<void> _salvarECompartilharCsv(BuildContext context, String csvData,
